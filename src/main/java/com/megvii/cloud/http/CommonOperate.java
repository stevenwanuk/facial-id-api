...
/**
 * 这个类里的所有方法都是网络请求，所以请在异步线程中调用 All the function in class is HTTP request, so
 * please call in an asynchronous thread
 */
public class CommonOperate {
	 ...
	 * @param apiKey
	 * @param apiSecret
	 * @param isInternationalVersion
	 *            是否是使用国际版 is use international version
	 */
	...
	/**
	 * 这是一个网络请求的方法，传入不同的参数和API接口，可以请求不同的API this is a HTTP request function, you can
	 * call all API through it
	 * 
	 * @param url
	 *            API接口 API url
	 * @param map
	 *            字符串参数 parameter of string
	 * @param fileByte
	 *            文件参数 parameter of file
	 * @return
	 * @throws Exception
	 */
	public static Response postTo(String url, HashMap<String, String> map, HashMap<String, byte[]> fileByte)
			throws Exception {
	 ...
	 *            图片链接 image url
	 * @param landmark
	 *            是否返回人脸的关键点，1：返回，0：不返回 Whether or return 83 key points of facial
	 *            features and contour，1：return，0：not return
	 * @param attributes
	 *            检测人脸的属性 gender,age,smiling,glass,headpose,facequality,blur detect
	 *            face attributes:
	 ...
	 * @throws Exception
	 */
	public Response compare(String faceToken1, String image_url1, byte[] fileByte1, String base64_1, String faceToken2,
			String image_url2, byte[] fileByte2, String base64_2) throws Exception {
	 ...
	 * faceToken,image_url,image_file,buff四个参数只要传入一个就可以了，其他可以传空（null） only need one
	 * of faceToken,image_url,image_file,buff
	 * 
	 * @param faceToken
	 ...
	 *            返回比对置信度最高的n个结果，范围[1,5]。默认值为1 the number of result, 1-5,Defaults to
	 *            1
	 * @return
	 * @throws Exception
	 */
	public Response searchByFaceSetToken(String faceToken, String image_url, byte[] buff, String faceSetToken,
			int returnResultCount) throws Exception {
	 ...
	 * @throws Exception
	 */
	public Response searchByOuterId(String faceToken, String image_url, byte[] buff, String base64, String outerId,
			int returnResultCount) throws Exception {
		String url = webUrl + Key.SPLIT + Key.SEARCH;
		HashMap<String, String> map = new HashMap<>();
		HashMap<String, byte[]> fileMap = new HashMap<>();
